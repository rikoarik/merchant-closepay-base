...
const PreviewQuickAccessButtons = memo<{
  ...
  // Responsive gap - smaller on small screens, larger on tablets
  const gap = useMemo(() => {
    const baseGap = scale(12);
    // Ensure gap is responsive but not too small
    return Math.max(scale(8), Math.min(baseGap, scale(16)));
  }, []);
  const itemsPerRow = 4;
    ...
    <>
      <View style={headerContainerStyle}>
        <TouchableOpacity
          ...
          activeOpacity={0.7}
        >
          <ArrowLeft2 size={getIconSize('medium')} color={backIconColor}  />
        </TouchableOpacity>
