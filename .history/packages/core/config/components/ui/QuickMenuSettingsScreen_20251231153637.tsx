...
// Static Preview Quick Access Buttons - Minimal hooks, optimized to prevent re-renders
const PreviewQuickAccessButtons = memo<{
  ...
  // Responsive gap - consistent dengan perhitungan width
  const gap = scale(12);
  const itemsPerRow = 4;
  ...
  // Only re-render if buttons, textColor, or buttonWidth actually changed
  if (
...
const PreviewContent = memo<{
  ...
  // Check other props (skip onBack as it's a function and might change reference)
  if (
...
export const QuickMenuSettingsScreen: React.FC = () => {
  ...
  // Icon mapping function for preview - smaller size
  const getPreviewMenuIcon = useCallback((iconName?: string) => {
    ...
    // Create new icon with smaller size for preview
  ...
  // Icon mapping function - memoized with useCallback and icon cache (for main menu)
  ...
  // Store previous previewButtons to maintain stable reference
  ...
  const previewButtons = useMemo(() => {
    // If key hasn't changed, return previous buttons to maintain reference stability
  ...
  // Memoize back icon color
  const backIconColor = useMemo(() => colors.text, [colors.text]);

  // Memoize text color for preview buttons
  const previewTextColor = useMemo(() => colors.text, [colors.text]);

  // Memoize menu container style with white background and rounded corners
  ...
  const previewButtonWidth = useMemo(() => {
    ...
    const cardPadding = scale(10); // Padding dari card container (previewCardContainer)
