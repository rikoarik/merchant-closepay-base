...
export const BottomSheet: React.FC<BottomSheetProps> = ({
  ...
  // Extract RGB dan base opacity dari colors.backdrop
  // colors.backdrop format: rgba(0, 0, 0, 0.5) atau rgba(0, 0, 0, 0.7)
  const { backdropColor, backdropBaseOpacity } = useMemo(() => {
    const rgbaMatch = colors.backdrop.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)(?:,\s*([\d.]+))?\)/);
    if (rgbaMatch) {
      const r = rgbaMatch[1];
      const g = rgbaMatch[2];
      const b = rgbaMatch[3];
      ...
      return {
        backdropColor: `rgb(${r}, ${g}, ${b})`,
        backdropBaseOpacity: baseOpacity,
      };
    }
    // Fallback ke hitam solid dengan opacity 0.5
    return {
      backdropColor: '#000000',
      backdropBaseOpacity: 0.5,
    };
  }, [colors.backdrop]);

  // Interpolate opacity animation dengan base opacity dari theme
  const animatedBackdropOpacity = useMemo(() => {
    return opacity.interpolate({
      inputRange: [0, 1],
      outputRange: [0, backdropBaseOpacity],
    });
...
const styles = StyleSheet.create({
  ...
  backdrop: {
    ...
    // backgroundColor akan di-set inline dengan colors.backdrop
  },
  sheet: {
    position: 'absolute',
    left: 0,
