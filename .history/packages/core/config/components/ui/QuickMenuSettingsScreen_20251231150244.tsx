...
export const QuickMenuSettingsScreen: React.FC = () => {
  ...
  useEffect(() => {
    const loadSettings = async () => {
      try {
        // Use getAllMenuItems to get menu from storage + plugin routes
        const allItems = await getAllMenuItems();
        console.log('Loaded menu items:', allItems.length, allItems);
  ...
  return (
    ...
    >
      ...
      <ScrollView
        ...
        showsVerticalScrollIndicator={false}
      >
        {!isLoading && !hasEnabledItems && menuItems.length > 0 ? (
